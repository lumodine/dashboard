...
import updateProductVariant from "@/actions/productVariant/updateProductVariant";

export type UpdateProductVariantFormProps = {
  tenant: any;
  variant: any;
};

export const UpdateProductVariantForm = ({tenant, variant}: UpdateProductVariantFormProps) => {
    ...
    <form action={clientAction} className="flex flex-col gap-4">
      <div className="grid gap-2">
        ...
        <div className="pl-3 mt-2 flex flex-col gap-2">
          {tenant.languages.map((language: any, languageIndex: number) => {
            const item = variant.translations.find(
      ...
      <div className="grid gap-2">
        ...
        <div className="pl-3 mt-2 flex flex-col gap-2">
          {tenant.currencies.map((currency: any, currencyIndex: number) => {
            const item = variant?.prices?.find(
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-1">
        <span className="text-muted-foreground text-xs">
          <b>Creation date</b>: {formatDate(variant.createdAt)}
        </span>
        <span className="text-muted-foreground text-xs">
          <b>Update date</b>: {formatDate(variant.updatedAt)}
        </span>
      </div>
...
UpdateProductVariantForm.displayName = "UpdateProductVariantForm";
